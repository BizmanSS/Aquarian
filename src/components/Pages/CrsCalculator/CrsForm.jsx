...
const CrsForm = () => {
  ...
  useEffect(() => {
    if (answers.length != 0)
    {calculateScore(); } // Recalculate the score whenever answers change
  }, [answers]);
  ...
  const validateAndCalculateScore = () => {
      ...
      Object.entries(answers).forEach(([questionId, answerValue]) => {
          if (!answerValue || answerValue === "Default") {
              newErrors[questionId] = true; // Mark this field as having an error
  ...
  return (
        <div className = "form-container">
            ...
            {showFinalScore && (
                <div className="final-score-display">Final Score: {score}</div>
            )}
        </div>
  );
};
